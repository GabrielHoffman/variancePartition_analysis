...
resList <- mclapply(assayNames(pb), function(CT){
  ...
  design <- model.matrix(form, data.frame(colData(pb)[keep,]))
  d <- DGEList(countMat[,keep],)
  d <- normLibSizes(d)
...
# Plots
```{r plot.vp, fig.height=7, fig.width=10}
library(cowplot)
...
figList <- lapply( names(resList), function(CT){
  ...
  fig2 <- plotTrendVP( dds, vp, "CountNoise" )
  ...
  # log2 CPM
  # libSize <-  matrix(colSums(counts(dds)), 
  #   nrow = nrow(counts(dds)), 
  #   ncol = ncol(counts(dds)),
  #   by = TRUE)
  # geneExpr <- log2(counts(dds) + 1) - log2(libSize) + log(1e6)

  # # log2 CPM
  # geneExpr <- log2(t(t(counts(dds) + 1) / sizeFactors(dds)))

  # log2 CPM
  geneExpr <- edgeR::cpm(counts(dds), log = TRUE)

  # Source
  g <-  genes["Source"]
  df <-  data.frame(y = geneExpr[g,], colData(dds))
  fig4 <- plotStratify(
    formula = y ~ Source, 
    data = df,
